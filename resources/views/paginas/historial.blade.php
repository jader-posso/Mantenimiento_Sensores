...
<head>
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .btn-export{display:flex;align-items:center;gap:.5rem;background:rgba(244,162,97,.1);border:1px solid rgba(244,162,97,.3);color:var(--gold);padding:.6rem 1.2rem;border-radius:4px;font-family:var(--font-mono);font-size:.75rem;letter-spacing:.1em;cursor:pointer;text-decoration:none;transition:background .2s;}
        .btn-export:hover{background:rgba(244,162,97,.2);}
        ...
        .nivel-ok{color:var(--teal);font-family:var(--font-mono);}
        .nivel-warn{color:var(--gold);font-family:var(--font-mono);}
        .nivel-err{color:var(--red);font-family:var(--font-mono);}
        .badge{font-family:var(--font-mono);font-size:.6rem;letter-spacing:.1em;padding:.25rem .6rem;border-radius:2px;}
        ...
        .badge-teal{background:rgba(46,196,182,.1);color:var(--teal);border:1px solid rgba(46,196,182,.3);}
...
<aside>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="/historial"        class="nav-item active"><span class="nav-icon">📊</span> Historial</a>
    </nav>
...
<main>
    <div class="page-top">
        <div>
            <h1 class="greeting">HISTORIAL DE <span>LECTURAS</span></h1>
            <p class="date">Registro de datos enviados por el sistema de monitoreo</p>
        </div>
    </div>
            ...
            <table>
                ...
                <tbody>
                    @foreach($lecturas as $lectura)
                        <tr>
                            <td class="td-mono">{{ \Carbon\Carbon::parse($lectura->Fecha_lectura)->format('Y-m-d H:i') }}</td>
                            <td>{{ $lectura->vehiculo->Nombre_vehiculo }}</td>
                            <td>{{ $lectura->sensor->Nombre_sensor }}</td>
                            <td class="td-mono">{{ $lectura->sensor->Tipo_sensor }}</td>
                            <td class="{{ $lectura->Estado == 'falla' ? 'nivel-err' : ($lectura->Estado == 'advertencia' ? 'nivel-warn' : 'nivel-ok') }}">{{ $lectura->Nivel }}%</td>
                            <td>
                                @if($lectura->Estado == 'falla')
                                    <span class="badge badge-red">FALLA</span>
                                @elseif($lectura->Estado == 'advertencia')
    ...
    @else
        <div class="empty-state">
            <div class="icon">📭</div>
            Aún no hay lecturas registradas para tus vehículos.
        </div>
    @endif
